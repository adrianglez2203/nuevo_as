...
class Fotos_de_album(models.Model):
    boda = models.CharField(max_length=30,blank=False)
    foto=models.ImageField(upload_to='',blank=False,null=None,)
    fotos=models.ForeignKey(Album_fotos_variadas, on_delete=models.CASCADE)
...
class Testimonio(models.Model):
    ...
    testimonio = models.TextField(max_length=500, null=False, blank=False)
    foto = models.ImageField(upload_to='')
    fecha_creacion = models.DateTimeField(auto_now_add=True)
...
class fotos_portafolio(models.Model):
    imagen_reducida = models.ImageField(upload_to='',blank=False)
    imagen_grande = models.ImageField(upload_to='',blank=False)
    descripcion = models.CharField(max_length=25,blank=True,default='Foto',help_text='Ingrese una breve descripción opcional')
